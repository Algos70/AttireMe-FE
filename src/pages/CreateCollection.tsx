...
const CreateCollection: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col items-center py-10 px-2">
      <div className="w-full max-w-3xl bg-white rounded-2xl shadow-xl border border-black/10 p-8">
        <h1 className="text-3xl font-bold text-black mb-6 text-center">Create New Collection</h1>
        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="flex flex-col items-center mb-6">
            ...
            {imagePreview && (
              <img src={imagePreview} alt="Preview" className="rounded-xl border border-indigo-300 shadow-lg max-w-full max-h-[400px] object-contain" />
            )}
          </div>
          <div>
            <label className="block text-black font-semibold mb-2">Collection Title</label>
            <input name="title" value={form.title} onChange={handleChange} required className="w-full border border-indigo-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-400 bg-white text-black" />
          </div>
          <div>
            <label className="block text-black font-semibold mb-2">Description</label>
            <textarea name="description" value={form.description} onChange={handleChange} required rows={3} className="w-full border border-indigo-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-400 bg-white text-black" />
          </div>
          <div>
            ...
            {genresLoading ? (
              ...
              <div className="flex flex-wrap gap-3">
                {genres.map(g => (
                  <label key={g.ID} className={`px-4 py-2 rounded-full border cursor-pointer ${form.genres.includes(g.ID) ? 'bg-indigo-500 text-white border-indigo-500' : 'bg-white text-black border-indigo-200'}`}>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-3">
              {SEASONS.map(s => (
                <label key={s} className={`px-4 py-2 rounded-full border cursor-pointer ${form.seasons.includes(s) ? 'bg-indigo-500 text-white border-indigo-500' : 'bg-white text-black border-indigo-200'}`}>
          ...
          <div>
            <label className="block text-black font-semibold mb-4 text-xl">Outfits</label>
            <div className="space-y-8">
              {form.outfits.map((outfit, idx) => (
                <div key={idx} className="border border-indigo-300 rounded-2xl p-6 bg-indigo-50/30 shadow flex flex-col gap-4 relative">
                  <div className="flex justify-between items-center mb-2">
                    <span className="font-semibold text-black text-lg">Outfit #{idx + 1}</span>
                    {form.outfits.length > 1 && (
                      <button type="button" onClick={() => handleRemoveOutfit(idx)} className="text-red-500 hover:underline text-sm font-medium">Remove</button>
                    )}
                  </div>
                  <div>
                    ...
                    <input value={outfit.description} onChange={e => setForm(f => {
                      ...
                      return { ...f, outfits };
                    })} className="w-full border border-indigo-200 rounded px-3 py-2 bg-white text-black" />
                  </div>
                  <div>
                    ...
                    {outfit.imageURL && <img src={outfit.imageURL} alt="Outfit Preview" className="mt-2 rounded-xl border border-indigo-200 max-h-56 object-contain mx-auto" />}
                  </div>
                  <div>
                    <label className="block text-black mb-2 font-medium">Outfit Items</label>
                    <div className="space-y-3">
                      {outfit.outfitItems.map((item, itemIdx) => (
                        <div key={itemIdx} className="bg-white/80 p-4 rounded-lg border border-indigo-100 flex flex-col gap-2 mb-2">
                          <label className="text-black font-medium mb-1">Store Name</label>
                          <input type="text" placeholder="Store Name" value={item.storeName} onChange={e => handleOutfitItemChange(idx, itemIdx, 'storeName', e.target.value)} className="border border-indigo-200 rounded px-2 py-1 bg-white text-black" />
                          <label className="text-black font-medium mb-1 mt-2">Product Link</label>
                          <input type="text" placeholder="Product Link" value={item.productLink} onChange={e => handleOutfitItemChange(idx, itemIdx, 'productLink', e.target.value)} className="border border-indigo-200 rounded px-2 py-1 bg-white text-black" />
                          ...
                          {item.imageURL && <img src={item.imageURL} alt="Item Preview" className="rounded border border-indigo-200 max-h-20 max-w-[120px] object-contain mt-2" />}
                      ...
                      <button type="button" onClick={() => handleAddOutfitItem(idx)} className="mt-2 px-4 py-2 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600 font-semibold">Add Item</button>
                    </div>
                  </div>
                </div>
              ))}
              <button type="button" onClick={handleAddOutfit} className="w-full mt-2 px-4 py-3 bg-black text-white rounded-xl hover:bg-indigo-600 transition font-bold text-lg">Add Outfit</button>
            </div>
          </div>
          {error && <div className="text-red-500 text-center">{error}</div>}
          <button type="submit" disabled={loading} className="w-full py-4 rounded-xl font-bold text-xl bg-indigo-500 text-white hover:bg-black transition disabled:opacity-60 disabled:cursor-not-allowed mt-4">
